...
setTimeout(async () => {
  ...
  // 1. Extract the theme directly from your _data object
  // Path: _data -> l (array) -> [0] (first item) -> data -> theme
  const themeFromData = _data.l[0].data.theme; 

  console.log(`[Runner] Found theme in data: "${themeFromData}"`);

  // 2. Setup listener (optional logging)
  ce_listen("msg", async(_$) => {
     console.log(`ce_listen`, _$);
  });

  // 3. Emit the message using the theme from your data
  await ce_call("msg", {
    type: "theme:change",
    custom: { theme: themeFromData }, // <--- Passing the value from _data
    _p: {}, 
    _$p: {} // Adding this ensures no errors if internal logic checks for it
  });
 ...
 setTimeout(async () => {
  ...
  await ce_call("msg", {
    type: "product:open",  // <--- The trigger type
    custom: { 
