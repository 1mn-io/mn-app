const { defineComponent, ref, computed, onMounted, onUnmounted, openBlock, createElementBlock, normalizeClass, unref, createElementVNode, normalizeStyle, Fragment, renderList, toDisplayString, createCommentVNode, createTextVNode, reactive, createBlock, createApp } = window.Vue;
const _hoisted_1$1 = ["id"], _hoisted_2 = ["onClick"], _hoisted_3 = ["src", "alt"], _hoisted_4 = { class: "absolute inset-0 flex flex-col justify-end md:justify-center px-6 pb-12 md:pb-0 md:px-16 max-w-7xl mx-auto w-full" }, _hoisted_5 = {
  ...
  setup(e) {
    const o = e, t = o._$p;
    console.log(o._$p);
    const r = ref(0), n = ref(null), a = ref(0), l = ref(0), u = o._p.f.name("vue-root"), b = (s) => {
      console.log(s);
      try {
        const c = s[t.data.curr.data.map.onClick];
        if (!c)
      ...
      const s = t.data.curr.data.value.l;
      return Array.isArray(s) && s.length > 0 ? s : k;
    }), k = [
      /* { 
           title: "Summer 2024", 
      ...
      a.value = s.changedTouches[0].screenX, g();
    }, N = (s) => {
      l.value = s.changedTouches[0].screenX, V(), v();
    }, V = () => {
      l.value < a.value - 50 && w(), l.value > a.value + 50 && y();
    }, v = () => {
    ...
    return onMounted(() => {
      ...
      createElementVNode("div", {
        ...
        (openBlock(!0), createElementBlock(Fragment, null, renderList(m.value, (i, _) => (openBlock(), createElementBlock("div", {
          ...
          c[1] || (c[1] = createElementVNode("div", { class: "absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent md:bg-gradient-to-r md:from-black/70 md:via-black/20 md:to-transparent" }, null, -1)),
          createElementVNode("div", _hoisted_4, [
            createElementVNode("div", {
              ...
              createElementVNode("h2", _hoisted_6, toDisplayString(i[unref(t).data.curr.data.map.title]), 1),
              createElementVNode("p", _hoisted_7, toDisplayString(i[unref(t).data.curr.data.map.desc]), 1),
              i[unref(t).data.curr.data.map.ctaText] ? (openBlock(), createElementBlock("button", {
                ...
                c[0] || (c[0] = createElementVNode("svg", {
  ...
  for (const [r, n] of o)
    t[r] = n;
  return t;
}, Carousel = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-884e4e5f"]]);
function d(e, o) {
  return o.split(".").reduce((t, r) => t?.[r], e);
}
function f(e, o) {
  const { open: t, close: r } = o, n = [];
  let a = 0;
  for (; a < e.length; ) {
    const l = e.indexOf(t, a);
    if (l === -1) {
      n.push({ type: "text", value: e.slice(a) });
      break;
    }
    l > a && n.push({ type: "text", value: e.slice(a, l) });
    const u = e.indexOf(r, l + t.length);
    if (u === -1)
      throw new Error(`Unclosed template placeholder starting at ${l}`);
    const b = e.slice(l + t.length, u).trim(), [m, k] = b.split("=");
    n.push({ type: "param", value: m, defaultValue: k }), a = u + r.length;
  }
  return n;
}
function p(e, o, t) {
  const r = /* @__PURE__ */ new Set(), n = e.map((a) => {
    if (a.type === "text") return a.value;
    const l = d(o, a.value);
    if (r.add(a.value.split(".")[0]), l === void 0 || l === "") {
      if (a.defaultValue !== void 0) return a.defaultValue;
      if (t.strict) throw new Error(`Missing param: ${a.value}`);
      return "";
    }
    return t.encode ? encodeURIComponent(String(l)) : String(l);
  }).join("");
  if (t.strict) {
...
const _hoisted_1 = ["id"], _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const props = __props, _$p = reactive(props._$p), rootId = computed(() => props._p.f.name("entry-root")), _ready = ref(!1);
    (async () => {
      try {
        ...
        try {
          const o = h(JSON.stringify(_$p.data.curr.data.api.headers), { open: "<", close: ">" })({
            localStorage: {
              token: localStorage.getItem("token") || ""
            }
          });
          _headers = JSON.parse(o);
        } catch (e) {
        }
        let _o = {
    ...
    return (e, o) => (openBlock(), createElementBlock("div", { id: rootId.value }, [
      _ready.value ? (openBlock(), createBlock(Carousel, {
        key: 0,
        _p: __props._p,
        _$p,
        _$cb: carouselCallbacks
      }, null, 8, ["_p", "_$p"])) : createCommentVNode("", !0)
    ], 8, _hoisted_1));
  }
}), index = async (e) => ({
  set: async (o, t) => {
    ...
    }, l = document.getElementById(e.f.name("vue-root"));
    return n.mount(l), a;
  }
});
export {
