...
const I = async (e) => {
        ...
        for (const o of r) {
          const i = await await m.lib.get({ name: o.type, run_from: "renderer", lazy_lib: e.lazy_lib }), c = await (await i.lib.index({
            f: {
              ...
              set_theme: async (s) => await (await g()).set(s),
  ...
  return await m.lib.set({ lib: e.lib, run_from: "hydrator", lazy_lib: e.lazy_lib }), {
    set: async (t) => {
      ...
      const i = async () => {
        for (const c of o) {
          const s = await await m.lib.get({ name: c.type, run_from: "hydrator", lazy_lib: e.lazy_lib }), d = s.lib, _ = y(), f = _.on, z = await (await d.index({
